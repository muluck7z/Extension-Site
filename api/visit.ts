...
async function sendVisitLog(request: Request): Promise<Response> {
  ...
  try {
    ...
    if (!discordResponse.ok) {
      console.error(`Discord visit webhook returned HTTP ${discordResponse.status}`);
    ...
    console.error("Discord visit webhook request failed", error);
    return new Response(JSON.stringify({ error: "Analytics delivery failed" }), {
      status: 502,
      headers: { "Content-Type": "application/json", "Cache-Control": "no-store" },
    });
